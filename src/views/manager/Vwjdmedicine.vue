<template>
  <div>
    ...
    <div style="margin: 10px 0">
      ...
      <el-button type="primary" plain @click="changeStateBatch()"  :disabled="ids.length <= 0">批量取药</el-button>
      <el-button type="primary" @click="load(1)">查询</el-button>
    </div>

    <el-table :data="vwjdmedicines" stripe border @selection-change="handleSelectionChange">
      ...
      <el-table-column label="药品名" prop="name" width="300" align="center"></el-table-column>
...
<!--      </el-table-column>-->
<!--      <el-table-column label="开药编号" prop="oid" width="70" align="center"></el-table-column>-->
<!--      <el-table-column label="药品名" prop="name" width="150" align="center"></el-table-column>-->
<!--      <el-table-column label="病人名" prop="realname" width="60" align="center"></el-table-column>-->
<!--      <el-table-column label="医生名" prop="dname" width="60" align="center"></el-table-column>-->
<!--      <el-table-column label="药品数量" prop="mnumber" width="70" align="center"></el-table-column>-->
<!--      <el-table-column label="开药时间" prop="ptime" width="90" align="center"></el-table-column>-->
<!--      <el-table-column label="取药"  width="80" align="center">-->
<!--        <template v-slot="scope">-->
<!--          <el-button type="primary" plain @click="changeState(scope.row.oid)" v-if ="scope.row.state=='未取'" >取药</el-button>-->
<!--          <el-button type="primary" plain disabled v-if ="scope.row.state=='已取'">取药</el-button>-->
<!--        </template>-->
<!--      </el-table-column>-->
<!--      <el-table-column label="取药状态" prop="state" width="70" align="center"></el-table-column>-->
<!--      <el-table-column label="操作" align="center">-->
<!--        <template v-slot="scope">-->
<!--          <el-button size="mini" type="danger" plain @click="del(scope.row.oid)">删除</el-button>-->
<!--        </template>-->
...
export default {
  ...
  created() {
    this.load();
    this.getJDMedicineName();
    this.getUserName();
    this.getDoctorName();
  },
  // mounted() {
  //   request.get('/Vwjdmedicine/selectAll').then(res => {
  //     this.vwjdmedicines = res.data
  //   })
  // },
  methods: {
    load(pageNum) {
      ...
      this.$request.get('/Vwjdmedicine/selectByPage', {
        params: {
        ...
        this.vwjdmedicines = res.data.records
        console.log(this.vwjdmedicines)
    ...
    getJDMedicineName() {
      this.$request.get('/jdmedicine/selectAll')
          .then(res => {
            if (res.code === '200') {
              this.jdmedicines = [];
              res.data.forEach(item => {
                this.jdmedicines.push({
                  mid: item.id,
                  medname: item.name
                });
              });
    ...
    changeState(oid) {//取药
      this.$confirm('确认取药吗？', '确认取药', {type: "warning"}).then(response => {
        this.$request.put('/Vwjdmedicine/state/' + oid).then(res => {
    ...
    changeStateBatch() {
      ...
      this.$confirm('您确认已取这些药品吗？', '确认取药', {type: "warning"}).then(response => {
        this.$request.put('/Vwjdmedicine/states/batch', this.ids).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            //重新加载页面
            this.load(1)
    ...
    del(oid) {
      this.$confirm('您确认删除吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/Vwjdmedicine/delete/' + oid).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    delBatch() {
      ...
      this.$confirm('您确认批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/Vwjdmedicine/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
